...
const CODE_SYMBOL = Symbol("@bufbuild/typescript-composer/code");
...
export interface Code {
  ...
  readonly indention: number;
  toString(): string;
  transform(t: Transformer): Code;
  with(...input: CodeLine[]): Code;
  tail(): Code;
}

export function isCode(v: unknown): v is Code {
  return isObjectWith(v, CODE_SYMBOL);
}

export function code(input: CodeInput, indention?: number): Code;
export function code(...input: [CodeInput]): Code;
export function code(...input: [CodeInput, number]): Code;
...
export function code(
  ...
  const newInput: CodeLine[] = [];
  for (const part of input) {
    if (isCode(part) && part.indention === 0)
...
function tag(
  ...
  let currentIndention = checkIndention(
  ...
  let currentCodeNode: Code = new CodeNode([]);
  ...
  for (let i = 0; i < segs.length; ++i) {
    ...
    for (let j = 0; j < additionalCodeNode.length; ++j) {
      ...
      const newIndention = checkIndention(additionalLine);
    ...
    if (i < params.length) {
      ...
      const lastPart = currentLine.segments.length
        ? currentLine.segments.slice(-1)[0]
        : undefined;
...
function checkIndention(s: string): number {
  return s.search(/([^ ]|$)/); // get position of first non-space character (or end)
}

type CodeLine = Code | CodeSequence;
export type CodeInput = Code | CodeLine[];

export function isCodeInput(input: unknown): input is CodeInput {
  return (
    isCodePart(input) ||
    (Array.isArray(input) && input.every(isCodePart))
  );
}

function isCodePart(input: unknown): input is CodeInput {
  return isCode(input) || isCodeSequence(input);
}

class CodeNode implements Code {
  readonly [CODE_SYMBOL] = true as const
  ...
  get indention() { return this.#indention; }
  ...
  with(...input: CodeLine[]) {
    return code([...this.lines, ...input], this.indention);
  }
